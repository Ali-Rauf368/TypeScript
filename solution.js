...
console.log("-----Task 2 ");
function endsWithIng(input) {
    var EndInput = input.toLowerCase();
...
console.log("-----Task 4 ");
function isFirstLetterLowerCase(input) {
    if (input.length === 0) {
        return false;
    }
 ...
 */
console.log("-----Task 5 ");
function cheaperThan(prices, limit) {
    var resultArray = [];
...
function wordsThatEndWithIng(words) {
    return words.filter(function (word) { return word.toLowerCase().endsWith("ing"); });
}
// Test cases
...
function onlyLongWords(sentence) {
    return sentence.split(" ").filter(function (str) { return str.length >= 5; }).join(" ");
}
// Test cases
...
function addLeadingZero(numbers) {
    return numbers.map(function (num) { return num.toString().padStart(2, '0'); });
}
 ...
 */
console.log("-----Task 10");
function cutStrings(sentences, maxLength) {
...
function durationInHours(minutes) {
    return minutes.map(function (duration) { return Math.floor(duration / 60); });
}
...
function reverseWords(words) {
    return words.map(function (word) { return word.split("").reverse().join(""); });
}
